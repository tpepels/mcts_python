...
def genetic_algorithm(
    ...
    for generation in range(num_generations):
        ...
        if not debug:
            ...
            # Use fake results for debugging
            results = []
            debug_fitness = [0] * len(population)

            for pair in pairs:
                fitness1 = random.randint(0, 2)
                fitness2 = random.randint(0, 2)
                results.append((pair[0], fitness1, pair[1], fitness2))

                debug_fitness[population.index(pair[0])] += fitness1
                debug_fitness[population.index(pair[1])] += fitness2
            print("Expected fitnesses: ", debug_fitness)
            print("Expected scaled fitnesses: ", scale_fitnesses(debug_fitness))
